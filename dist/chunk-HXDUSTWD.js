...
function useGridLayout(options) {
  const {
    ...
    onLayoutChange,
    compactor = chunkQ7HHXBLO_js.verticalCompactor
  } = options;
  const isDraggingRef = react.useRef(false);
  const [layout, setLayoutState] = react.useState(() => {
    const corrected = chunkQ7HHXBLO_js.correctBounds(chunkQ7HHXBLO_js.cloneLayout(propsLayout), { cols });
  ...
  const setLayout = react.useCallback(
    (newLayout) => {
      const corrected = chunkQ7HHXBLO_js.correctBounds(chunkQ7HHXBLO_js.cloneLayout(newLayout), { cols });
  ...
  const onDragStart = react.useCallback(
    (itemId, x, y) => {
      const item = chunkQ7HHXBLO_js.getLayoutItem(layout, itemId);
      if (!item) return null;
      isDraggingRef.current = true;
      const placeholder = {
        ...chunkQ7HHXBLO_js.cloneLayoutItem(item),
        x,
        y,
        static: false,
        moved: false
      ...
      setDragState({
        ...
        oldDragItem: chunkQ7HHXBLO_js.cloneLayoutItem(item),
        oldLayout: chunkQ7HHXBLO_js.cloneLayout(layout)
      });
      return placeholder;
    },
    [layout]
  ...
  const onResizeStart = react.useCallback(
    (itemId) => {
      ...
      setResizeState({
        resizing: true,
        oldResizeItem: chunkQ7HHXBLO_js.cloneLayoutItem(item),
        oldLayout: chunkQ7HHXBLO_js.cloneLayout(layout)
      });
      return item;
    },
  ...
  const onResize = react.useCallback(
    (itemId, w, h, x, y) => {
      ...
      const corrected = chunkQ7HHXBLO_js.correctBounds(newLayout, { cols });
      const compacted = compactor.compact(corrected, cols);
      setLayoutState(compacted);
    },
  ...
  const onDropDragOver = react.useCallback(
    (droppingItem, position) => {
      const existingItem = chunkQ7HHXBLO_js.getLayoutItem(layout, droppingItem.i);
      if (!existingItem) {
  ...
  const containerHeight = react.useMemo(() => chunkQ7HHXBLO_js.bottom(layout), [layout]);
...
function useResponsiveLayout(options) {
  ...
  const sortedBreakpoints = react.useMemo(
    () => chunkQ7HHXBLO_js.sortBreakpoints(breakpoints),
    [breakpoints]
  );
  const initialBreakpoint = react.useMemo(
    () => chunkQ7HHXBLO_js.getBreakpointFromWidth(breakpoints, width),
    // Only calculate on mount, not on width changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );
  const initialCols = react.useMemo(
    () => chunkQ7HHXBLO_js.getColsFromBreakpoint(initialBreakpoint, colsConfig),
  ...
  const [layouts, setLayoutsState] = react.useState(() => {
    ...
    for (const bp of sortedBreakpoints) {
      ...
      if (layout2) {
        cloned[bp] = chunkQ7HHXBLO_js.cloneLayout(layout2);
      }
    }
    return cloned;
  });
  ...
  const setLayoutForBreakpoint = react.useCallback((bp, newLayout) => {
    setLayoutsState((prev) => ({
      ...prev,
      [bp]: chunkQ7HHXBLO_js.cloneLayout(newLayout)
    }));
  }, []);
  const setLayouts = react.useCallback((newLayouts) => {
    ...
    for (const bp of Object.keys(newLayouts)) {
      ...
      if (layoutForBp) {
        cloned[bp] = chunkQ7HHXBLO_js.cloneLayout(layoutForBp);
      }
    }
  ...
  react.useEffect(() => {
    ...
    const newBreakpoint = chunkQ7HHXBLO_js.getBreakpointFromWidth(breakpoints, width);
    const newCols = chunkQ7HHXBLO_js.getColsFromBreakpoint(newBreakpoint, colsConfig);
    onWidthChange?.(width, [10, 10], newCols, null);
    if (newBreakpoint !== breakpoint) {
      const newLayout = chunkQ7HHXBLO_js.findOrGenerateResponsiveLayout(
        layouts,
        breakpoints,
        newBreakpoint,
        breakpoint,
