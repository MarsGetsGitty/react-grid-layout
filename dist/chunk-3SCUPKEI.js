...
function useGridItemDrag(opts) {
  ...
  const onDragStart = React3.useCallback(
    (e, { node }) => {
      ...
      setDragging(true);
      const rawPos = chunkQ7HHXBLO_js.calcXYRaw(positionParams, newPosition.top, newPosition.left);
      const { x: newX, y: newY } = chunkQ7HHXBLO_js.applyPositionConstraints(
        constraints,
        effectiveLayoutItem,
        rawPos.x,
        rawPos.y,
  ...
  const onDrag = React3.useCallback(
    (e, { node, deltaX, deltaY }) => {
      ...
      if (dragPendingRef.current && !thresholdExceededRef.current) {
        ...
        if (onDragStartProp) {
          const rawPos2 = chunkQ7HHXBLO_js.calcXYRaw(
            positionParams,
            dragPositionRef.current.top,
            dragPositionRef.current.left
          );
          const { x: startX, y: startY } = chunkQ7HHXBLO_js.applyPositionConstraints(
            constraints,
            effectiveLayoutItem,
            rawPos2.x,
            rawPos2.y,
      ...
      if (isBounded) {
        const { offsetParent } = node;
        if (offsetParent) {
          const bottomBoundary = offsetParent.clientHeight - chunkQ7HHXBLO_js.calcGridItemWHPx(h, rowHeight, margin[1]);
          top = chunkQ7HHXBLO_js.clamp(top, 0, bottomBoundary);
          const colWidth = chunkQ7HHXBLO_js.calcGridColWidth(positionParams);
          const rightBoundary = containerWidth - chunkQ7HHXBLO_js.calcGridItemWHPx(w, colWidth, margin[0]);
          left = chunkQ7HHXBLO_js.clamp(left, 0, rightBoundary);
        }
      }
      const newPosition = { top, left };
      dragPositionRef.current = newPosition;
      const rawPos = chunkQ7HHXBLO_js.calcXYRaw(positionParams, top, left);
      const { x: newX, y: newY } = chunkQ7HHXBLO_js.applyPositionConstraints(
        constraints,
        effectiveLayoutItem,
        rawPos.x,
...
function useGridItemResize(opts) {
  ...
  const onResizeHandler = React3.useCallback(
    (e, { node, size, handle: resizeHandle }, position, handlerName) => {
      ...
      let updatedSize;
      if (node) {
        updatedSize = chunkQ7HHXBLO_js.resizeItemInDirection(
          resizeHandle,
          position,
          size,
          containerWidth
      ...
      const rawSize = chunkQ7HHXBLO_js.calcWHRaw(positionParams, updatedSize.width, updatedSize.height);
      const { w: newW, h: newH } = chunkQ7HHXBLO_js.applySizeConstraints(
        constraints,
        effectiveLayoutItem,
        rawSize.w,
        rawSize.h,
  ...
  const handleResizeStart = React3.useCallback(
    (e, data) => {
      setResizing(true);
      const pos = chunkQ7HHXBLO_js.calcGridItemPosition(positionParams, x, y, w, h);
      const typedData = {
        ...data,
        handle: data.handle
      };
...
function GridItem(props) {
  const {
    ...
    resizeHandles,
    resizeHandle,
    constraints = chunkQ7HHXBLO_js.defaultConstraints,
    layoutItem,
    layout = [],
  ...
  const createStyle = React3.useCallback(
    (pos2) => {
      ...
      if (useCSSTransforms) {
        return chunkQ7HHXBLO_js.setTransform(pos2);
      }
      const styleObj = chunkQ7HHXBLO_js.setTopLeft(pos2);
      if (usePercentages) {
        return {
          ...styleObj,
          left: chunkQ7HHXBLO_js.perc(pos2.left / containerWidth),
          width: chunkQ7HHXBLO_js.perc(pos2.width / containerWidth)
        };
      }
      return styleObj;
    },
  ...
  const pos = chunkQ7HHXBLO_js.calcGridItemPosition(
    positionParams,
    x,
    y,
    w,
  ...
  const minConstraints = [
    chunkQ7HHXBLO_js.calcGridItemWHPx(minW, colWidth, margin[0]),
    chunkQ7HHXBLO_js.calcGridItemWHPx(minH, rowHeight, margin[1])
  ];
  const maxConstraints = [
    chunkQ7HHXBLO_js.calcGridItemWHPx(maxW, colWidth, margin[0]),
    chunkQ7HHXBLO_js.calcGridItemWHPx(maxH, rowHeight, margin[1])
  ];
  ...
  if (ghostDrag && dragging && gridContainerRef?.current) {
    const ghostPos = chunkQ7HHXBLO_js.calcGridItemPosition(
      positionParams,
      x,
      y,
      w,
...
function useGridLayoutDrag(opts) {
  ...
  const onDragStart = React3.useCallback(
    (i, _x, _y, data) => {
      ...
      const l = chunkQ7HHXBLO_js.getLayoutItem(currentLayout, i);
      if (!l) return;
      const placeholder = {
        w: l.w,
        h: l.h,
      ...
      oldDragItemRef.current = chunkQ7HHXBLO_js.cloneLayoutItem(l);
  ...
  const onDrag = React3.useCallback(
    (i, x, y, data) => {
      ...
      if (collisionResolver) {
        const tentative = chunkQ7HHXBLO_js.moveElement(
          currentLayout,
          l,
          x,
          y,
        ...
        const movedItem = chunkQ7HHXBLO_js.getLayoutItem(tentative, i);
        if (!movedItem) return;
      ...
      const newLayout = chunkQ7HHXBLO_js.moveElement(
        currentLayout,
        l,
        x,
        y,
...
function useGridLayoutResize(opts) {
  ...
  const onResizeStart = React3.useCallback(
    (i, _w, _h, data) => {
      ...
      const l = chunkQ7HHXBLO_js.getLayoutItem(currentLayout, i);
      if (!l) return;
      oldResizeItemRef.current = chunkQ7HHXBLO_js.cloneLayoutItem(l);
      oldLayoutRef.current = currentLayout;
      setResizing(true);
  ...
  const onResize = React3.useCallback(
    (i, w, h, data) => {
      ...
      let newX;
      let newY;
      const [newLayout, l] = chunkQ7HHXBLO_js.withLayoutItem(currentLayout, i, (item) => {
        newX = item.x;
        newY = item.y;
        ...
        if (preventCollision && !allowOverlap) {
          const collisions = chunkQ7HHXBLO_js.getAllCollisions(currentLayout, {
            ...item,
            w,
            h,
            x: newX ?? item.x,
      ...
      if (shouldMoveItem && newX !== void 0 && newY !== void 0) {
        finalLayout = chunkQ7HHXBLO_js.moveElement(
          newLayout,
          l,
          newX,
          newY,
...
function useGridLayoutDrop(opts) {
  ...
  const handleDragOver = React3.useCallback(
    (e) => {
      ...
      const actualColWidth = chunkQ7HHXBLO_js.calcGridColWidth(positionParams);
      const itemPixelWidth = chunkQ7HHXBLO_js.calcGridItemWHPx(
        finalDroppingItem.w,
        actualColWidth,
        margin[0]
      );
      const itemPixelHeight = chunkQ7HHXBLO_js.calcGridItemWHPx(
        finalDroppingItem.h,
        rowHeight,
        margin[1]
      );
      ...
      if (!droppingDOMNode) {
        const calculatedPosition = chunkQ7HHXBLO_js.calcXY(
          positionParams,
          clampedGridY,
          clampedGridX,
          finalDroppingItem.w,
...
function synchronizeLayoutWithChildren(initialLayout, children, cols, compactor) {
  ...
  React3__default.default.Children.forEach(children, (child) => {
    ...
    if (existingItem) {
      layout.push(chunkQ7HHXBLO_js.cloneLayoutItem(existingItem));
    } else {
      const childProps = child.props;
      ...
      if (dataGrid) {
        ...
        layout.push({
          i: key,
          x: 0,
          y: chunkQ7HHXBLO_js.bottom(layout),
          w: 1,
          h: 1
        });
      }
  ...
  const corrected = chunkQ7HHXBLO_js.correctBounds(layout, { cols });
...
function GridLayout(props) {
  const {
    ...
    positionStrategy = chunkQ7HHXBLO_js.defaultPositionStrategy,
  ...
  const gridConfig = React3.useMemo(
    () => ({ ...chunkQ7HHXBLO_js.defaultGridConfig, ...gridConfigProp }),
    [gridConfigProp]
  );
  const dragConfig = React3.useMemo(
    () => ({ ...chunkQ7HHXBLO_js.defaultDragConfig, ...dragConfigProp }),
    [dragConfigProp]
  );
  const resizeConfig = React3.useMemo(
    () => ({ ...chunkQ7HHXBLO_js.defaultResizeConfig, ...resizeConfigProp }),
    [resizeConfigProp]
  );
  const dropConfig = React3.useMemo(
    () => ({ ...chunkQ7HHXBLO_js.defaultDropConfig, ...dropConfigProp }),
    [dropConfigProp]
  );
  ...
  const compactor = compactorProp ?? chunkQ7HHXBLO_js.getCompactor("vertical");
  ...
  const containerHeight = React3.useMemo(() => {
    ...
    const nbRow = chunkQ7HHXBLO_js.bottom(layout);
  ...
  const processGridItem = React3.useCallback(
    (child, isDroppingItem) => {
      ...
      const l = chunkQ7HHXBLO_js.getLayoutItem(layout, String(child.key));
      if (!l) return null;
...
function ResponsiveGridLayout(props) {
  ...
  const initialBreakpoint = React3.useMemo(() => {
    return propBreakpoint ?? chunkQ7HHXBLO_js.getBreakpointFromWidth(breakpoints, width);
  }, []);
  const initialCols = React3.useMemo(() => {
    return chunkQ7HHXBLO_js.getColsFromBreakpoint(initialBreakpoint, colsConfig);
  }, [initialBreakpoint, colsConfig]);
  const initialLayout = React3.useMemo(() => {
    return chunkQ7HHXBLO_js.findOrGenerateResponsiveLayout(
      propsLayouts,
      breakpoints,
      initialBreakpoint,
      initialBreakpoint,
  ...
  React3.useEffect(() => {
    if (compactType !== prevCompactTypeRef.current) {
      const newLayout = compactor.compact(chunkQ7HHXBLO_js.cloneLayout(effectiveLayout), cols);
      const newLayouts = {
        ...layoutsRef.current,
  ...
  React3.useEffect(() => {
    ...
    if (widthChanged || breakpointPropChanged || breakpointsChanged || colsChanged) {
      const newBreakpoint = propBreakpoint ?? chunkQ7HHXBLO_js.getBreakpointFromWidth(breakpoints, width);
      const newCols = chunkQ7HHXBLO_js.getColsFromBreakpoint(newBreakpoint, colsConfig);
      ...
      if (lastBreakpoint !== newBreakpoint || breakpointsChanged || colsChanged) {
        ...
        if (!newLayouts[lastBreakpoint]) {
          newLayouts[lastBreakpoint] = chunkQ7HHXBLO_js.cloneLayout(layout);
        }
        let newLayout = chunkQ7HHXBLO_js.findOrGenerateResponsiveLayout(
          newLayouts,
          breakpoints,
          newBreakpoint,
          lastBreakpoint,
      ...
      const currentMargin2 = chunkQ7HHXBLO_js.getIndentationValue(
        propMargin,
        newBreakpoint
      );
      const currentPadding = propContainerPadding ? chunkQ7HHXBLO_js.getIndentationValue(
  ...
  const currentMargin = React3.useMemo(() => {
    return chunkQ7HHXBLO_js.getIndentationValue(
      propMargin,
      breakpoint
    );
