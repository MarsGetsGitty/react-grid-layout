...
function calculateOverlap(source, target, axis) {
  if (axis === "vertical") {
...
function resolveAxisCollisions(layout, source, axis, direction, maxRows, cols, visited) {
  const collisions = getAllCollisions(layout, source).filter((item) => item.i !== source.i && !visited.has(item.i));
...
function resolveResizeCollisions(layout, resizedId, oldItem, newItem, maxRows, cols) {
  ...
  if (vertical) {
    ...
    const visited = /* @__PURE__ */ new Set([resizedId]);
