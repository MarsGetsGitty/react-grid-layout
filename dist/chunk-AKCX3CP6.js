...
function useGridItemDrag(opts) {
  ...
  const onDragStart = React3.useCallback(
    (e, { node }) => {
      ...
      setDragging(true);
      const rawPos = chunkTNAOBXMI_js.calcXYRaw(positionParams, newPosition.top, newPosition.left);
      const { x: newX, y: newY } = chunkTNAOBXMI_js.applyPositionConstraints(
        constraints,
        effectiveLayoutItem,
        rawPos.x,
        rawPos.y,
  ...
  const onDrag = React3.useCallback(
    (e, { node, deltaX, deltaY }) => {
      ...
      if (dragPendingRef.current && !thresholdExceededRef.current) {
        ...
        if (onDragStartProp) {
          const rawPos2 = chunkTNAOBXMI_js.calcXYRaw(
            positionParams,
            dragPositionRef.current.top,
            dragPositionRef.current.left
          );
          const { x: startX, y: startY } = chunkTNAOBXMI_js.applyPositionConstraints(
            constraints,
            effectiveLayoutItem,
            rawPos2.x,
            rawPos2.y,
      ...
      if (isBounded) {
        const { offsetParent } = node;
        if (offsetParent) {
          const bottomBoundary = offsetParent.clientHeight - chunkTNAOBXMI_js.calcGridItemWHPx(h, rowHeight, margin[1]);
          top = chunkTNAOBXMI_js.clamp(top, 0, bottomBoundary);
          const colWidth = chunkTNAOBXMI_js.calcGridColWidth(positionParams);
          const rightBoundary = containerWidth - chunkTNAOBXMI_js.calcGridItemWHPx(w, colWidth, margin[0]);
          left = chunkTNAOBXMI_js.clamp(left, 0, rightBoundary);
        }
      }
      const newPosition = { top, left };
      dragPositionRef.current = newPosition;
      const rawPos = chunkTNAOBXMI_js.calcXYRaw(positionParams, top, left);
      const { x: newX, y: newY } = chunkTNAOBXMI_js.applyPositionConstraints(
        constraints,
        effectiveLayoutItem,
        rawPos.x,
...
function useGridItemResize(opts) {
  ...
  const onResizeHandler = React3.useCallback(
    (e, { node, size, handle: resizeHandle }, position, handlerName) => {
      ...
      let updatedSize;
      if (node) {
        updatedSize = chunkTNAOBXMI_js.resizeItemInDirection(
          resizeHandle,
          position,
          size,
          containerWidth
      ...
      const rawSize = chunkTNAOBXMI_js.calcWHRaw(positionParams, updatedSize.width, updatedSize.height);
      const { w: newW, h: newH } = chunkTNAOBXMI_js.applySizeConstraints(
        constraints,
        effectiveLayoutItem,
        rawSize.w,
        rawSize.h,
  ...
  const handleResizeStart = React3.useCallback(
    (e, data) => {
      setResizing(true);
      const pos = chunkTNAOBXMI_js.calcGridItemPosition(positionParams, x, y, w, h);
      const typedData = {
        ...data,
        handle: data.handle
      };
...
function GridItem(props) {
  const {
    ...
    resizeHandles,
    resizeHandle,
    constraints = chunkTNAOBXMI_js.defaultConstraints,
    layoutItem,
    layout = [],
  ...
  const createStyle = React3.useCallback(
    (pos2) => {
      ...
      if (useCSSTransforms) {
        return chunkTNAOBXMI_js.setTransform(pos2);
      }
      const styleObj = chunkTNAOBXMI_js.setTopLeft(pos2);
      if (usePercentages) {
        return {
          ...styleObj,
          left: chunkTNAOBXMI_js.perc(pos2.left / containerWidth),
          width: chunkTNAOBXMI_js.perc(pos2.width / containerWidth)
        };
      }
      return styleObj;
    },
  ...
  const pos = chunkTNAOBXMI_js.calcGridItemPosition(
    positionParams,
    x,
    y,
    w,
  ...
  const minConstraints = [
    chunkTNAOBXMI_js.calcGridItemWHPx(minW, colWidth, margin[0]),
    chunkTNAOBXMI_js.calcGridItemWHPx(minH, rowHeight, margin[1])
  ];
  const maxConstraints = [
    chunkTNAOBXMI_js.calcGridItemWHPx(maxW, colWidth, margin[0]),
    chunkTNAOBXMI_js.calcGridItemWHPx(maxH, rowHeight, margin[1])
  ];
  ...
  if (ghostDrag && dragging && gridContainerRef?.current) {
    const ghostPos = chunkTNAOBXMI_js.calcGridItemPosition(
      positionParams,
      x,
      y,
      w,
...
function useGridLayoutDrag(opts) {
  ...
  const onDragStart = React3.useCallback(
    (i, _x, _y, data) => {
      ...
      const l = chunkTNAOBXMI_js.getLayoutItem(currentLayout, i);
      if (!l) return;
      const placeholder = {
        w: l.w,
        h: l.h,
      ...
      oldDragItemRef.current = chunkTNAOBXMI_js.cloneLayoutItem(l);
      oldLayoutRef.current = currentLayout.map((item) => chunkTNAOBXMI_js.cloneLayoutItem(item));
      latestDragLayoutRef.current = currentLayout.map((item) => chunkTNAOBXMI_js.cloneLayoutItem(item));
  ...
  const onDrag = React3.useCallback(
    (i, x, y, data) => {
      ...
      if (collisionResolver) {
        const tentativeBase = currentLayout.map((item) => chunkTNAOBXMI_js.cloneLayoutItem(item));
        const tentativeItem = chunkTNAOBXMI_js.getLayoutItem(tentativeBase, i);
        if (!tentativeItem) return;
        const tentative = chunkTNAOBXMI_js.moveElement(
          tentativeBase,
          tentativeItem,
          x,
          y,
        ...
        const movedItem = chunkTNAOBXMI_js.getLayoutItem(tentative, i);
        if (!movedItem) return;
        ...
        if (resolved) {
          ...
          const acceptedItem = chunkTNAOBXMI_js.getLayoutItem(compacted2, i) ?? movedItem;
          onDragProp(compacted2, oldDragItem, acceptedItem, placeholder2, data.e, data.node);
        } else {
          const eventItem = chunkTNAOBXMI_js.getLayoutItem(currentLayout, i) ?? l;
      ...
      const newLayout = chunkTNAOBXMI_js.moveElement(
        currentLayout,
        l,
        x,
        y,
...
function useGridLayoutResize(opts) {
  ...
  const onResizeStart = React3.useCallback(
    (i, _w, _h, data) => {
      ...
      const l = chunkTNAOBXMI_js.getLayoutItem(currentLayout, i);
      if (!l) return;
      oldResizeItemRef.current = chunkTNAOBXMI_js.cloneLayoutItem(l);
  ...
  const onResize = React3.useCallback(
    (i, w, h, data) => {
      ...
      let newX;
      let newY;
      const [newLayout, l] = chunkTNAOBXMI_js.withLayoutItem(currentLayout, i, (item) => {
        newX = item.x;
        newY = item.y;
        ...
        if (preventCollision && !allowOverlap) {
          const collisions = chunkTNAOBXMI_js.getAllCollisions(currentLayout, {
            ...item,
            w,
            h,
            x: newX ?? item.x,
      ...
      if (shouldMoveItem && newX !== void 0 && newY !== void 0) {
        finalLayout = chunkTNAOBXMI_js.moveElement(
          newLayout,
          l,
          newX,
          newY,
...
function useGridLayoutDrop(opts) {
  ...
  const handleDragOver = React3.useCallback(
    (e) => {
      ...
      const actualColWidth = chunkTNAOBXMI_js.calcGridColWidth(positionParams);
      const itemPixelWidth = chunkTNAOBXMI_js.calcGridItemWHPx(
        finalDroppingItem.w,
        actualColWidth,
        margin[0]
      );
      const itemPixelHeight = chunkTNAOBXMI_js.calcGridItemWHPx(
        finalDroppingItem.h,
        rowHeight,
        margin[1]
      );
      ...
      if (!droppingDOMNode) {
        const calculatedPosition = chunkTNAOBXMI_js.calcXY(
          positionParams,
          clampedGridY,
          clampedGridX,
          finalDroppingItem.w,
...
function synchronizeLayoutWithChildren(initialLayout, children, cols, compactor) {
  ...
  React3__default.default.Children.forEach(children, (child) => {
    ...
    if (existingItem) {
      layout.push(chunkTNAOBXMI_js.cloneLayoutItem(existingItem));
    } else {
      const childProps = child.props;
      ...
      if (dataGrid) {
        ...
        layout.push({
          i: key,
          x: 0,
          y: chunkTNAOBXMI_js.bottom(layout),
          w: 1,
          h: 1
        });
      }
  ...
  const corrected = chunkTNAOBXMI_js.correctBounds(layout, { cols });
...
function GridLayout(props) {
  const {
    ...
    positionStrategy = chunkTNAOBXMI_js.defaultPositionStrategy,
  ...
  const gridConfig = React3.useMemo(
    () => ({ ...chunkTNAOBXMI_js.defaultGridConfig, ...gridConfigProp }),
    [gridConfigProp]
  );
  const dragConfig = React3.useMemo(
    () => ({ ...chunkTNAOBXMI_js.defaultDragConfig, ...dragConfigProp }),
    [dragConfigProp]
  );
  const resizeConfig = React3.useMemo(
    () => ({ ...chunkTNAOBXMI_js.defaultResizeConfig, ...resizeConfigProp }),
    [resizeConfigProp]
  );
  const dropConfig = React3.useMemo(
    () => ({ ...chunkTNAOBXMI_js.defaultDropConfig, ...dropConfigProp }),
    [dropConfigProp]
  );
  ...
  const compactor = compactorProp ?? chunkTNAOBXMI_js.getCompactor("vertical");
  ...
  const containerHeight = React3.useMemo(() => {
    ...
    const nbRow = chunkTNAOBXMI_js.bottom(layout);
  ...
  const processGridItem = React3.useCallback(
    (child, isDroppingItem) => {
      ...
      let l = chunkTNAOBXMI_js.getLayoutItem(layout, String(child.key));
...
function ResponsiveGridLayout(props) {
  ...
  const initialBreakpoint = React3.useMemo(() => {
    return propBreakpoint ?? chunkTNAOBXMI_js.getBreakpointFromWidth(breakpoints, width);
  }, []);
  const initialCols = React3.useMemo(() => {
    return chunkTNAOBXMI_js.getColsFromBreakpoint(initialBreakpoint, colsConfig);
  }, [initialBreakpoint, colsConfig]);
  const initialLayout = React3.useMemo(() => {
    return chunkTNAOBXMI_js.findOrGenerateResponsiveLayout(
      propsLayouts,
      breakpoints,
      initialBreakpoint,
      initialBreakpoint,
  ...
  React3.useEffect(() => {
    if (compactType !== prevCompactTypeRef.current) {
      const newLayout = compactor.compact(chunkTNAOBXMI_js.cloneLayout(effectiveLayout), cols);
      const newLayouts = {
        ...layoutsRef.current,
  ...
  React3.useEffect(() => {
    ...
    if (widthChanged || breakpointPropChanged || breakpointsChanged || colsChanged) {
      const newBreakpoint = propBreakpoint ?? chunkTNAOBXMI_js.getBreakpointFromWidth(breakpoints, width);
      const newCols = chunkTNAOBXMI_js.getColsFromBreakpoint(newBreakpoint, colsConfig);
      ...
      if (lastBreakpoint !== newBreakpoint || breakpointsChanged || colsChanged) {
        ...
        if (!newLayouts[lastBreakpoint]) {
          newLayouts[lastBreakpoint] = chunkTNAOBXMI_js.cloneLayout(layout);
        }
        let newLayout = chunkTNAOBXMI_js.findOrGenerateResponsiveLayout(
          newLayouts,
          breakpoints,
          newBreakpoint,
          lastBreakpoint,
      ...
      const currentMargin2 = chunkTNAOBXMI_js.getIndentationValue(
        propMargin,
        newBreakpoint
      );
      const currentPadding = propContainerPadding ? chunkTNAOBXMI_js.getIndentationValue(
  ...
  const currentMargin = React3.useMemo(() => {
    return chunkTNAOBXMI_js.getIndentationValue(
      propMargin,
      breakpoint
    );
