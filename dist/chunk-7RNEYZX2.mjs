...
function calcMaxRows(containerHeight, rowHeight, marginY, paddingY) {
  const availableHeight = Math.max(0, containerHeight - paddingY * 2);
  return availableHeight > 0 ? Math.floor((availableHeight + marginY) / (rowHeight + marginY)) : Infinity;
}
...
function toGridUnits(fraction, totalUnits) {
...
var containerBounds = {
  ...
  constrainPosition(item, x, y, { cols, maxRows, containerHeight, rowHeight, margin, containerPadding }) {
    const visibleRows = containerHeight > 0 ? calcMaxRows(containerHeight, rowHeight, margin[1], containerPadding[1]) : maxRows;
    return {
      x: clamp2(x, 0, Math.max(0, cols - item.w)),
...
// src/core/math/adaptive-metrics.ts
var ADAPTIVE_DEFAULTS = Object.freeze({
  targetCellWidth: 120,
  minRowHeight: 50,
  minCols: 6,
  maxCols: 24,
  margin: Object.freeze([6, 6]),
  containerPadding: null
});
function computeAdaptiveMetrics(containerWidth, containerHeight, options = {}) {
  ...
  const rawColWidth = (containerWidth - margin[0] * (cols - 1) - effectivePadding[0] * 2) / cols;
...
function resolveOptions(input) {
  return {
    ...
    containerPadding: input.containerPadding !== void 0 ? input.containerPadding : ADAPTIVE_DEFAULTS.containerPadding
  };
}

// src/core/math/proportional.ts
function toProportional(item, ctx) {
...
function fromProportional(frac, ctx) {
  const { cols, maxRows } = ctx;
  if (cols <= 0 || maxRows <= 0) {
...
function toProportionalLayout(layout, ctx) {
  return layout.map((item) => ({
    i: item.i,
    ...toProportional(item, ctx)
  }));
}
function fromProportionalLayout(proportional, ctx, originals) {
...
function totalOccupiedRows(layout) {
  let max = 0;
  for (const item of layout) {
    const bottom2 = item.y + item.h;
    if (bottom2 > max) max = bottom2;
  }
  return max;
}

// src/core/math/proportional-repair.ts
function repairLayout(layout, ctx, constraints) {
  const { cols, maxRows } = ctx;
  ...
  for (const item of items) {
    ...
    if (c.maxW !== void 0 && item.w > c.maxW) item.w = c.maxW;
    if (c.maxH !== void 0 && item.h > c.maxH) item.h = c.maxH;
  ...
  for (const item of items) {
    ...
    while (collision !== void 0 && budget > 0) {
...
function hasOverlaps(layout) {
  for (let i = 0; i < layout.length; i++) {
    for (let j = i + 1; j < layout.length; j++) {
      const a = layout[i];
      const b = layout[j];
      if (a !== void 0 && b !== void 0 && collides(a, b)) {
        return true;
      }
    }
  }
...
function hasOverflow(layout, ctx) {
  for (const item of layout) {
...
function findFirstCollision(placed, candidate) {
...
function hasNewlyInvalidItems(resolvedLayout, previousLayout, maxRows) {
  if (maxRows === Infinity) return false;
  const prevArray = previousLayout;
...
var pcdCollisionResolver = (tentativeLayout, movedItem, originalPosition, context) => {
  ...
  const previousLayout = context.previousLayout ?? tentativeLayout;
  ...
  if (maxRows !== Infinity) {
    ...
    if (dragged.y + dragged.h > maxRows) {
      dragged.y = Math.max(0, maxRows - dragged.h);
    }
    if (dragged.y < 0) {
      dragged.y = 0;
    }
...
export { ADAPTIVE_DEFAULTS, absoluteStrategy, applyPositionConstraints, applySizeConstraints, aspectRatio, bottom, boundedX, boundedY, calcGridCellDimensions, calcGridColWidth, calcGridItemPosition, calcGridItemWHPx, calcMaxRows, calcWH, calcWHRaw, calcXY, calcXYRaw, clamp, cloneLayout, cloneLayoutItem, collides, computeAdaptiveMetrics, containerBounds, correctBounds, createPhysicsEngine, createScaledStrategy, defaultConstraints, defaultDragConfig, defaultDropConfig, defaultGridConfig, defaultPositionStrategy, defaultResizeConfig, findOrGenerateResponsiveLayout, fromProportional, fromProportionalLayout, getAllCollisions, getBreakpointFromWidth, getColsFromBreakpoint, getCompactor, getFirstCollision, getIndentationValue, getLayoutItem, getStatics, gridBounds, hasOverflow, hasOverlaps, horizontalCompactor, horizontalOverlapCompactor, inferResizeHandles, maxSize, minMaxSize, minSize, modifyLayout, moveElement, moveElementAwayFromCollision, noCompactor, noOverlapCompactor, pcdCollisionResolver, perc, repairLayout, resizeItemInDirection, resolveResizeCollisions, setTopLeft, setTransform, snapToGrid, sortBreakpoints, sortLayoutItems, sortLayoutItemsByColRow, sortLayoutItemsByRowCol, toGridUnits, toProportional, toProportionalLayout, totalOccupiedRows, transformStrategy, trySwap, validateLayout, verticalCompactor, verticalOverlapCompactor, withLayoutItem };
